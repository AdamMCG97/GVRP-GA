...
public class Algorithm {
    ...
    public static Population evolvePop(Population population) {
        ...
        int elitismAmount = (population.size() / 10);
        ...
        for (int i = elitismOffset; i < nextGeneration.size(); i++) {
            ...
            nextGeneration.sort(i + 1);
            if (binarySearch.search(nextGeneration.getArray(), 0, i, child)) {
                i = i - 1;
            } else {
                nextGeneration.saveRoute(i, child);
            }
            nextGeneration.sort(i + 1);
        }
        //mutate all individuals in new population
        for (int i = elitismOffset; i < nextGeneration.size(); i++) {
            mutate(nextGeneration.getRoute(i));
        }
        ...
        if (elitism) {
            ...
            nextGeneration.saveRoute(0, population.getFittest());
            //System.out.println("test");
            for (int i = 1; i < elitismAmount; i++) {
                nextGeneration.saveRoute(i, population.getRoute(population.size() - i));
            }
            //offset pointer for new population correctly
            elitismOffset = elitismAmount;
        }

        //population finished evolving, reapply charging stations where necessary to new population
        for (int i = 0; i < population.size(); i++) {
            nextGeneration.getRoute(i).includeRecharge();
        }
